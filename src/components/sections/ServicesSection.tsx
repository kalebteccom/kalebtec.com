  ...
  return (
    <section id="services" aria-label="Our Services" className="relative py-32">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <SectionHeading title="WHAT WE DO" sectionNumber="02" />
          ...
          {services.map((service, index) => (
            <AnimatedReveal key={service.name} delay={0.1 * index}>
              ...
              >
                {/* Card index number */}
                <span
                  className="absolute top-4 right-4 font-mono text-[11px] text-cyber-faint/40"
                  aria-hidden="true"
                >
                ...
                {/* Description */}
                <p className="text-sm leading-relaxed text-cyber-muted">{service.description}</p>
              </div>
            </AnimatedReveal>
          ))}
        </div>
